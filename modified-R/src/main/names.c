...
SEXP attribute_hidden do_internal(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    R_Visible = flag != 1;
	if (TYPEOF(INTERNAL(fun)) == BUILTINSXP)
	  ans = RRnR_after(s, INTERNAL(fun), args, env, RRNR_BUILTIN, RRnR_before(s, INTERNAL(fun), args, env, RRNR_BUILTIN) ? PRIMFUN(INTERNAL(fun)) (s, INTERNAL(fun), args, env) : R_NilValue);
	else

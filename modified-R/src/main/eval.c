...
SEXP eval(SEXP e, SEXP rho)
{
	...
	int hook_enabled = RRnR_eval_before(e, rho);
	...
	if(hook_enabled) RRnR_eval_after(e, rho);
	return e;
    default: break;
    }
	...
	if (TYPEOF(op) == SPECIALSXP) {
		...
		R_Srcref = NULL;
		tmp = RRnR_after(e, op, tmp, rho, RRNR_BUILTIN, RRnR_before(e, op, tmp, rho, RRNR_BUILTIN) ? PRIMFUN(op) (e, op, tmp, rho) : R_NilValue);
		R_Srcref = oldref;
		endcontext(&cntxt);
	    } else {
		tmp = RRnR_after(e, op, tmp, rho, RRNR_BUILTIN, RRnR_before(e, op, tmp, rho, RRNR_BUILTIN) ? PRIMFUN(op) (e, op, tmp, rho) : R_NilValue);
                                   ...
                                   SEXP rho, SEXP arglist, SEXP op)
{
    ...
    body = BODY(op);
	if (R_CheckJIT(op) && RRnR_JIT_before(call)) {
	int old_enabled = R_jit_enabled;
	SEXP newop;
	R_jit_enabled = 0;
	newop = R_cmpfun(op);
    ...
    if ((RDEBUG(op) && R_current_debug_state()) || RSTEP(op)
		...
		int browser = RRnR_browser_before();
		if(browser != 0)
		{
			dbg = TRUE;
			SET_RSTEP(op, 0);
		...
		if(browser != -1) RRnR_browser_after();
    }
...
#define DO_DOTCALL() do {						\
	...
	if (ofun && nargs <= DOTCALL_MAX) {				\
	    ...
	    void *vmax = vmaxget();					\
		SEXP val = RRnR_after(call, R_NilValue, R_NilValue, rho, RRNR_DOTCALL, RRnR_before(call, R_NilValue, R_NilValue, rho, RRNR_DOTCALL) ? R_doDotCall(ofun, nargs, cargs, call) : R_NilValue);		\
	    vmaxset(vmax);						\
	...
	SEXP sym = CAR(call);						\
	SEXP op = getPrimitive(sym, BUILTINSXP);			\
	SETSTACK(-1, RRnR_after(call, op, args, rho, RRNR_DOTCALL, RRnR_before(call, op, args, rho, RRNR_DOTCALL) ? do_dotcall(call, op, args, rho) : R_NilValue));			\
	R_Visible = TRUE;						\
	NEXT();								\
    } while (0)
...
//#define REPORT_OVERRIDEN_BUILTINS
	...
	case BUILTINSXP:
	  ...
	  flag = PRIMPRINT(fun);
	  R_Visible = flag != 1;
	  value = RRnR_after(call, fun, args, rho, RRNR_BUILTIN, RRnR_before(call, fun, args, rho, RRNR_BUILTIN) ? PRIMFUN(fun) (call, fun, args, rho) : R_NilValue);
	  if (flag < 2) R_Visible = flag != 1;
	  break;
	case SPECIALSXP:
	  flag = PRIMPRINT(fun);

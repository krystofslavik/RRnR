...
create_barplot <- function(..., legend, col, ylab, ltitle)
{
  ...
  par(mar=c(10,4,2,4))
  par(lty = 0)
  lim_max <- 4#ceiling(max(all_results))
  ...
  text(cex=1, x=colMeans(bp)+0.8, y=lim_min-0.5, names(all_results)[-1], xpd=TRUE,
       srt=90, pos=2)
...
create_barplot <- function(rec_diff, rec_err_min, rec_err_max, rep_diff, rep_err_min, rep_err_max, legend, col, ylab, ltitle)
{
  ...
  par(mar=c(8,4,2,4))
  par(lty = 0)
  lim_max <- ceiling(max(max(rec_err_max), max(rec_err_min), max(rep_err_max), max(rep_err_min)) * 10) / 10 - 1
  lim_min <- floor(min(min(rec_err_max), min(rec_err_min), max(rep_err_max), max(rep_err_min)) * 10) / 10 - 1
  bp <- barplot(agg, beside=T, legend=legend, col=col, ylim=c(lim_min,lim_max),
  ...
  #abline(h=lim_min:lim_max, col = "gray60", lty=2)
  text(cex=1, x=colMeans(bp)+0.5, y=lim_min-0.0, colnames(agg), xpd=TRUE,
       srt=90, pos=2)
